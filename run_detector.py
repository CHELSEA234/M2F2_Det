...
## Hyper-params #######################
hparams = {
            'epochs': 200, 'batch_size': 32, 'basic_lr': 1e-3, 'fine_tune': True, 'use_laplacian': True, 'step_factor': 0.4, 
        ...
        if (tot_iter + 1) % hparams['valid_step'] == 0:
            ...
            with torch.no_grad():
                ...
                for idx, val_batch in tqdm(enumerate(val_generator, 1), total=len(val_generator), desc='valid'):
                    ...
                    val_img_batch = val_img_batch.float().to(device)
